            ...
            var HUAFILE = {
                ...
                html5_ajax_slice_upload: function(file) {
                    ...
                    return function() {
                        ...
                        xhr.onreadystatechange = function(e) {
                            if (xhr.readyState == 4) {
                                if (xhr.status == 200) {
                                    ...
                                    responseData = parseInt(xhr.responseText);
                                    if (0 !== responseData) {
                                        total_loaded += responseData;
                                    } else {
                                        alert('文件上传出错,请检查文件上传类型或刷新重新上传');
                                        clearInterval(HUAFILE.ajax_upload[file.index]); // 终止文件上传
                                        return '';
                                    }
                        ...
                        //数据对象                       
                        data = new FormData();
                        data.append('_csrf', option._csrf);
                        data.append('files', blob, file['name']);
                        // 文件唯一标识
                        data.append('fileName', file['time']);
                        data.append('originFileName', file['name']);
                        // 用于后台判断文件是否上传完
